...
def update_longest_streak(habit_name):
    # checks if new streak value is higher than longest_streak and updates longest_streak
    cur = db.cursor()
    streak = get_streak(habit_name)
    longest_streak = cur.execute("SELECT longest_streak WHERE Habit_name = ?", (habit_name,)).fetchone()
...
def get_all_habits_same_periodicity(timespan):
    ...
    db.commit()
    return habits_same_periodicity


def get_longest_streak_given_habit(habit_name):
    cur = db.cursor()
    longest_streak = cur.execute("SELECT longest_Streak FROM Streak WHERE Habit_name= ?", (habit_name,)).fetchall()
    db.commit()
